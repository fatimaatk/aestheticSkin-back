...
const getAllComments = (id) => {
    return new Promise((resolve, reject) => {
        dbConnect.query('SELECT rate_comment.rate_id, rate_comment.comment, rate_comment.product_id, products.title, user.email FROM rate_comment INNER JOIN products ON rate_comment.product_id = products.id INNER JOIN user ON rate_comment.user_id = user.id ORDER BY product_id', id, (err, results) => {
            if (err) reject(err);
